...
export const Header = () => {
  ...
  const itemBase =
    'flex-1 md:flex-none px-4 py-1.5 text-xs font-semibold transition-all duration-300 rounded-lg text-center whitespace-nowrap min-w-[60px] md:min-w-[80px]';
      ...
      <div className='container mx-auto px-6 py-4 flex flex-wrap justify-between items-center'>
        ...
        >
          <div className='w-5 h-4 flex flex-col justify-between'>
            <span
              className={`h-0.5 bg-current transition-all duration-300 origin-left ${isOpen ? 'rotate-[42deg] w-[26px]' : 'w-full'}`}
            />
            <span
              className={`h-0.5 bg-current transition-all duration-300 ${isOpen ? 'opacity-0' : 'w-full'}`}
            />
            <span
              className={`h-0.5 bg-current transition-all duration-300 origin-left ${isOpen ? '-rotate-[42deg] w-[26px]' : 'w-full'}`}
            />
